...
export default class App extends Component<Props> {
  ...
  checkBound(){
    ...
    wifi.connectionStatusOfBoundNetwork((isBound) => {
      if (isBound) {
          this.setState({checkBound: 'Network is bound'});
      } else {
          this.setState({checkBound: 'Network is not bound'});
      }
    });
  }
  ...
  deviceInfo() {
    ...
    fetch(`${ip}/fg?module`, { headers })
      .then(data => this.setState({ deviceInfo: JSON.stringify(data)})) // JSON-string from `response.json()` call
      .catch(error => this.setState({ deviceInfo: error }));
  }
  ...
  render() {
    ...
    <ScrollView>
      <View style={styles.container}>
        ...
        <View style={styles.instructionsContainer}>
          <Text style={styles.instructionsTitle}>Check if network is bound</Text>
          <View style={styles.row}>
            <TouchableHighlight style={styles.button} onPress={this.checkBound.bind(this)}>
              <Text style={styles.buttonText}>Check if network is bound</Text>
            </TouchableHighlight>
            <Text style={styles.answer}>{this.state.checkBound==null?"":this.state.checkBound}</Text>
          </View>
        </View>
        <View style={styles.instructionsContainer}>
          <Text style={styles.instructionsTitle}>Force WIFI</Text>
          <View style={styles.row}>
            <TouchableHighlight style={styles.button} onPress={this.forceWifi.bind(this)}>
              <Text style={styles.buttonText}>Force WIFI</Text>
            </TouchableHighlight>
            <Text style={styles.answer}>{this.state.forceWifi==null?"":this.state.forceWifi}</Text>
          </View>
        </View>
        <View style={styles.instructionsContainer}>
          <Text style={styles.instructionsTitle}>Contact device</Text>
          <View style={styles.row}>
            <TouchableHighlight style={styles.button} onPress={this.deviceInfo.bind(this)}>
              <Text style={styles.buttonText}>Get device info</Text>
            </TouchableHighlight>
            <Text style={styles.answer}>{this.state.deviceInfo==null?"":this.state.deviceInfo}</Text>
          </View>
        </View>
      </View>
      <Modal 
